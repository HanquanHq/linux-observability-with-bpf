...
def print_flow(pid, recv_bytes, send_bytes, sum_recv, sum_send, sum_kb):
    ...
    while i != args.count and not interrupted:
        ...
        if ipv4_throughput:
            for k, (send_bytes, recv_bytes) in sorted(ipv4_throughput.items(),
                ...
                print_flow(k.pid, recv_bytes, send_bytes, sum_recv, sum_send, sum_kb)
        else:
            print("%-10d %-12.12s %-10d %-10d %-10d %-10d %-10d" % (0, "-", 0, 0, sum_recv, sum_send, sum_kb))
        i += 1
